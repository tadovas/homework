...
public class CrawlerLogQueryControllerTest {
  ...
  @MockBean private CrawlerLogRepository repository;

  @Test
  public void shouldReturnAllUnfinishedLogsFromRepo() throws Exception {

    Mockito.when(repository.findAllUnfinished())
        .thenReturn(Arrays.asList(createNewLog("1"), createNewLog("2")));
  ...
  public void shouldReturnAllFinishedLogsFromRepoMatchingUrl() throws Exception {
    ...
    Mockito.when(repository.findAllFinishedWithMatchingSourceUrl(Mockito.any()))
        .thenReturn(Arrays.asList(createNewLog("3"), createNewLog("4")));
