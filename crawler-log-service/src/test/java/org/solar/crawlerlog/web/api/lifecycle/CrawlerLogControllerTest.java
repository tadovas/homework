...
public class CrawlerLogControllerTest {
  ...
  @MockBean private CrawlerLogService crawlerLogService;

  @Test
  public void shouldCreateNewLogForGivenUrlAndReturnWarningThatAnotherLogWithSameUrlExists()
      throws Exception {

    ArgumentCaptor<SourceUrl> sourceUrlCaptor = ArgumentCaptor.forClass(SourceUrl.class);
    when(crawlerLogService.createNewCrawlerLog(sourceUrlCaptor.capture()))
        .thenReturn(CreationResult.newResult(LogId.fromString("123"), true));
  ...
  public void shouldReturnFoundCrawlerLogWithRequiredLinksWhenFoundInRepository() throws Exception {
    ...
    when(crawlerLogService.findCrawlerLogById(logIdArgCaptor.capture()))
        .thenReturn(createTestCrawlerLog());

    mockMvc
        .perform(get("/crawler-logs/{id}", "123").accept(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8))
        .andExpect(jsonPath("$.id", equalTo("123")))
        .andExpect(jsonPath("$.sourceUrl", equalTo("test-url")))
        ...
        .andExpect(
            jsonPath(
                "$.links[?(@.rel=='self')].href", hasItem("http://localhost/crawler-logs/123")))
        .andExpect(
            jsonPath(
                "$.links[?(@.rel=='celebrities')].href",
                hasItem("http://localhost/crawler-logs/123/celebrities")))
        .andExpect(
            jsonPath(
                "$.links[?(@.rel=='repository')].href",
                hasItem("http://localhost/crawler-logs/123/repository")));
  ...
  public void shouldAddListOfCelebritiesToExistingLog() throws Exception {
    ...
    assertThat(
        celebritiesCaptor.getValue(),
        containsInAnyOrder(
            hasProperty("name", equalTo("Arnold")), hasProperty("name", equalTo("Elton"))));
  }
    ...
    mockMvc
        .perform(
            put("/crawler-logs/123/repository")
                .accept(MediaType.APPLICATION_JSON)
                .contentType(MediaType.APPLICATION_JSON)
                .content("{" + " \"repositoryId\" : \"remoteRepoId\" " + "}"))
        .andExpect(status().isAccepted());

    ArgumentCaptor<LogId> logIdCaptor = ArgumentCaptor.forClass(LogId.class);
    ArgumentCaptor<RemoteRepositoryId> repoIdCaptor =
        ArgumentCaptor.forClass(RemoteRepositoryId.class);
  ...
  public void anyActionOnNonExistentLogReturns404Error() throws Exception {
    ...
    doThrow(new CrawlerLogNotFoundException("Irrelevant"))
        .when(crawlerLogService)
        .finishCrawlerLog(any(), any());
  ...
  public void anyModificationOnFinishedLogShouldProduce409Error() throws Exception {
    doThrow(new LogAlreadyFinishedException("Irrelevant"))
        .when(crawlerLogService)
        .finishCrawlerLog(any(), any());
  ...
  /**
   * Unfortunatelly ArgumentCaptor does not support capturing generic lists very nicely, adding a
   * little workaround
   */
  ...
  private CrawlerLog createTestCrawlerLog() {
    CrawlerLog log =
        CrawlerLog.newCrawlerLog(LogId.fromString("123"), SourceUrl.fromString("test-url"));
    log.addCelebrities(
        Arrays.asList(new Celebrity("Arnold", "actor"), new Celebrity("Elton", "singer")));
    return log;
  }
}
